...
We can recursively build **extents** for each node to represent the union of bounding box for that node's and it's children. An extent is an array of three tuples: each tuple is the min/max pair for the X, Y, and Z axis coordinates, in that order.
